...
const Sekans = () => {
  ...
  const getProducts = () => {
    try {
      post('/api/admins/get-myorders', { id: admin?.dealerId, statu: 2 }).then((res) => {
        if (res.result) {
      ...
      {waiting == false ? (
        products?.length > 0 ? (
          <CTable align="middle" className="mb-0 border" hover responsive>
            <CTableHead color="light">
              <CTableRow>
                ...
                <CTableHeaderCell className="text-center">Sipariş Trh</CTableHeaderCell>
                <CTableHeaderCell className="text-center">Teslim Trh</CTableHeaderCell>
              </CTableRow>
            </CTableHead>
            <CTableBody>
              {openModel ? (
          ...
          <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
            <h2>Teslim edilen siparişiniz bulunmamaktadır</h2>
          </div>
        )
      ) : (
        <div style={{ width: '100%', display: 'flex', justifyContent: 'center' }}>
