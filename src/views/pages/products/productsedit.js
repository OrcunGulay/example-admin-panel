...
const Sekans = () => {
  ...
  const getProduct = (item) => {
    try {
      post('/api/admins/get-product', { item }).then((res) => {
        if (res.result) {
          setProduct(res.product)
          setProducts(res.products)
          setCategorie(res.product.productCategoriesId)
          setCategories(res.categories)
          setSelectedCategories(JSON.parse(res.product.colors))
          setSelectedSizes(JSON.parse(res.product.size))
          setSelectedProducts(JSON.parse(res.product.similar))
          setSelectedImage(JSON.parse(res.product.images)[0])
          setSelectedImage2(JSON.parse(res.product.images)[1])
          setSelectedImage3(JSON.parse(res.product.images)[2])
          setSelectedImage4(JSON.parse(res.product.images)[3])
        }
      })
    } catch (e) {
      console.log('hata')
  ...
  const postProduct = (name, price, discount, categorieId, colors, size, desc, similar) => {
    try {
      post('/api/admins/update-product', {
        ...
        if (res.result) {
          if (imageObject || imageObject2 || imageObject3 || imageObject4) {
            addToast(exampleToast('Resim yükleniyor...'))

            const formData = new FormData()
            formData.append('image', imageObject)
            formData.append('fileName', `products/${product?.productId}.1`)

            const formData2 = new FormData()
            formData2.append('image', imageObject2)
            formData2.append('fileName', `products/${product?.productId}.2`)

            const formData3 = new FormData()
            formData3.append('image', imageObject3)
            formData3.append('fileName', `products/${product?.productId}.3`)

            const formData4 = new FormData()
            formData4.append('image', imageObject4)
            formData4.append('fileName', `products/${product?.productId}.4`)
            ...
            addToast(exampleToast('Ürün Güncellendi'))
            window.location.href = `#/products`
          }
          setLoading(false)
        }
            ...
            {products?.map((_item, _index) => (
              <CFormCheck
                ...
                label={
                  <div>
                    <img
                      src={_item?.images ? JSON.parse(_item?.images)[0] : image}
                      alt={_item.name}
      ...
      <CButton
        onClick={() => {
          ...
          postProduct(
            ...
            categorie ? categorie : product.productCategoriesId,
            selectedCategories ? selectedCategories : product.color,
            selectedSizes ? selectedSizes : product.size,
            desc ? desc : product.desc,
            selectedProducts ? selectedProducts : products.similar,
          )
        }}
      >
        {loading ? (
